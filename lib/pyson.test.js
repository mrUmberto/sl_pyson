...
describe('pyson test', () => {
    ...
    it('Not-complex', () => {
        const expr = {
            "__class__": "Not",
            "v": {"s2": "to upgrade", "s1": {"d": "", "__class__": "Eval", "v": "state"}, "__class__": "Equal"}
        };
...
it('Eval list', () => {
    expect(pyson.resolve([{d: "", __class__: "Eval", v: "active_id"}],
        {active_ids: Array(1), active_id: 1, active_model: "party.party"}
    )).toEqual([1]);
});
...
it('Composite 17', () => {
    const expr = {
        "s": [{
            "k": {"v": "state", "d": "", "__class__": "Eval"},
            "__class__": "In",
            "v": ["cancel", "done"]
        }, {"__class__": "Bool", "v": {"v": "incoming_moves", "d": [0], "__class__": "Eval"}}, {
            "__class__": "Bool",
            "v": {"v": "inventory_moves", "d": [0], "__class__": "Eval"}
        }], "__class__": "Or"
    };
    const data = {
        state: 'draft',
    ...
    expect(pyson.resolve(expr, data1)).toBe(true);
    ...
    expect(pyson.resolve(expr, data2)).toBe(false);

    const data3 = {
        state: 'draft',
        incoming_moves: []
    };
    expect(pyson.resolve(expr, data3)).toBe(true);
});

it('Rare cases', () => {
   const expr = {"__class__": "Bool", "v": {"v": "invoiced_duration", "d": "", "__class__": "Eval"}};
   const data = {invoiced_duration: "00:00:00"};
   expect(pyson.resolve(expr, data)).toBe(false);
   const data1 = {invoiced_duration: 0};
   expect(pyson.resolve(expr, data1)).toBe(false);
   const data2 = {invoiced_duration: '01:00:00'};
   expect(pyson.resolve(expr, data2)).toBe(true);
});
